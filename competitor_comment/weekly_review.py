...
def generate_weekly_report(all_data, days):
    ...
    try:
        summary = run_task("weekly_review_report", context=weekly_ctx)
    except Exception as e:
        summary = f"AI 分析失败: {e}"

    # ── 本地化专题摘要 ──
    try:
        localization_insight = run_task("weekly_review_localization", context={
            "region_names": region_names,
            "localization_reviews": localization_reviews,
            "competitor_names": competitor_names,
        })
    except Exception as e:
